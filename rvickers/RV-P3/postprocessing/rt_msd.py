...
for pressure_diff in pressure_diffs:
    ...
    for array1,index in zip(rtDF,rtDF.index):
        for array2 in array1:
            rearr_rtDF.append([index,array2[0],array2[1],array2[2],array2[3],array2[4]])
    rearr_rtDF=pd.DataFrame(rearr_rtDF,columns=['mol','start','end','Residence time','avg absydiff','xy dist']).set_index('mol')
    ...
    rearr_rtDF.to_csv('./csv'+rt_dist+'/pagap'+gap_size+'_nve'+pressure_diff+'_rt.csv')
