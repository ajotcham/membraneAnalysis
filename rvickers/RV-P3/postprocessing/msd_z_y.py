...
for pressure_diff in pressure_diffs:
    ...
    for i,cut in enumerate(cuts):
        molDFi = molDF.copy()
        tubeyDF = molDFi.loc[:,idx[:,'tube']].droplevel(1,axis=1)[(molDFi.loc[:,idx[:,'y avg']].droplevel(1,axis=1) >= cut[0]) & (molDFi.loc[:,idx[:,'y avg']].droplevel(1,axis=1) < cut[1])].fillna(0)
        tubeyDF.columns = pd.MultiIndex.from_product([tubeyDF.columns, ['tube']])
        molDFi.loc[:,idx[:,'tube']] = tubeyDF
        molDFi.iloc[:,molDFi.columns.get_level_values(1)=='x'] = molDFi.T.apply(get_dis,dim='x').T
        molDFi.iloc[:,molDFi.columns.get_level_values(1)=='y'] = molDFi.T.apply(get_dis,dim='y').T
        molDFi.iloc[:,molDFi.columns.get_level_values(1)=='z'] = molDFi.T.apply(get_dis,dim='z').T
        
        SDDFz = molDFi.iloc[:,molDFi.columns.get_level_values(1)=='z'].rename({'z':'sdz'}, axis=1) ** 2
        SDDF = molDFi.iloc[:,molDFi.columns.get_level_values(1)=='x'].rename({'x':'sd'}, axis=1) ** 2 + molDFi.iloc[:,molDFi.columns.get_level_values(1)=='y'].rename({'y':'sd'}, axis=1) ** 2 + SDDFz.rename({'sdz':'sd'}, axis=1)
        molDFi = pd.concat([molDFi, SDDF, SDDFz], axis=1).sort_index(axis=1)

        MSDDF = molDFi.groupby(level=0,axis=1).apply(get_MSD,'sd').droplevel([0,1], axis=1).groupby(level=0,axis=1).sum()
        MSDDFz = molDFi.groupby(level=0,axis=1).apply(get_MSD,'sdz').droplevel([0,1], axis=1).groupby(level=0,axis=1).sum()
        MDDFz = molDFi.groupby(level=0,axis=1).apply(get_MSD,'z').droplevel([0,1], axis=1).groupby(level=0,axis=1).sum()
        MSDDF['avg'] = MSDDF['sum']/MSDDF['count']
        MSDDFz['avg'] = MSDDFz['sum']/MSDDFz['count']
        MDDFz['avg'] = MDDFz['sum']/MDDFz['count']

        MSDDF.to_csv('./csv_y'+str(i)+'/pagap'+gap_size+'_nve'+pressure_diff+'_molsMSDDF.csv')
        MSDDFz.to_csv('./csv_y'+str(i)+'/pagap'+gap_size+'_nve'+pressure_diff+'_molsMSDDF_z.csv')
        MDDFz.to_csv('./csv_y'+str(i)+'/pagap'+gap_size+'_nve'+pressure_diff+'_molsMDDF_z.csv')
