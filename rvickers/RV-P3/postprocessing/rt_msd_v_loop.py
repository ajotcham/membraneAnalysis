...
def get_residence_time_and_y_diff(x,rt_dist):
    ...
    for i in range(len(boundsDF)-1): 
        if (cutDF[boundsDF[i]] < cutDF[boundsDF[i+1]]):
            ...
            times.append([start, stop, stop-start, y_avg, tot_dist, v_avg])
    if not times:
        times=None
    return times
...
for filenum in range(0,12):
    ...
    for array1,index in zip(rtDF,rtDF.index):
        for array2 in array1:
            rearr_rtDF.append([index,array2[0],array2[1],array2[2],array2[3],array2[4],array2[5]])
    rearr_rtDF=pd.DataFrame(rearr_rtDF,columns=['mol','start','end','Residence time','avg absydiff', 'xy dist','avg v']).set_index('mol')
    rearr_rtDF.to_csv('./'+stage+'_rt/64x_'+gap_size+'_rt_v_'+str(filenum)+'.csv')
    print('finished: '+file)