...
for pressure_diff in pressure_diffs:
    ...
    MSDz = np.zeros((len(SDDF.values),3))
    MSD = np.zeros((len(SDDF.values),3))
    MD = np.zeros((len(SDDF.values),3))
    for bins, values, valuesz,values1 in zip(consecDF.values, SDDF.values,SDDFz.values, zDisDF.values):
        for binny, value, valuez, value1 in zip(bins,values,valuesz,values1):
            MSD[binny][0] += 1
            MSD[binny][1] += value
            MSDz[binny][0] += 1
            MSDz[binny][1] += valuez
    ...
    MSDDFz = pd.DataFrame(MSDz,columns=['count','sum','avg'])
    MDDF = pd.DataFrame(MD,columns=['count','sum','avg'])
    MSDDF.to_csv('./csv/pagap'+gap_size+'_nve'+pressure_diff+'_molsMSDDF.csv')
    MSDDFz.to_csv('./csv/pagap'+gap_size+'_nve'+pressure_diff+'_molsMSDDF_z.csv')
    MDDF.to_csv('./csv/pagap'+gap_size+'_nve'+pressure_diff+'_molsMDDF_z.csv')
