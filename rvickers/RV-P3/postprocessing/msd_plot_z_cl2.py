...
for pressure_diff in pressure_diffs:
    for i,cut in enumerate(cuts):
        ...
        clDF = (abs((molDF.iloc[:,molDF.columns.get_level_values(1)=='ycorr'].mean(axis=1)-cl) > cut[0]) & (abs(molDF.iloc[:,molDF.columns.get_level_values(1)=='ycorr'].mean(axis=1)-cl) < cut[1]))
        clMols = clDF.loc[clDF==True].index.values
        molDF=molDF.loc[clMols]
