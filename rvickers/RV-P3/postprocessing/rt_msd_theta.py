...
for pressure_diff in pressure_diffs:
    ...
    xcorrDF = (molDF.T.loc[idx[:,('x')],idx[:]]-offsetx*np.floor(molDF.T.loc[idx[:,('x')],idx[:]]/offsety)).T.rename({'x':'xcorr'},axis=1)
    print('pre-concat')
    ...
    # thetaxs,thetays,thetazs = molDF.T.apply(get_theta_hist,args=[int(rt_dist)]).dropna()
    # thetasDF = pd.DataFrame({'thetax':thetaxs, 'thetay':thetays,'thetaz':thetazs})
    rearr_rtDF = []
    ...
    rearr_rtDF.to_csv('./'+stage+'_csv_rt/64x_'+gap_size+'_rt_theta.csv')
    # thetasDF.to_csv('./csv'+rt_dist+'/pagap'+gap_size+'_nve'+pressure_diff+'_rt_hist.csv')
