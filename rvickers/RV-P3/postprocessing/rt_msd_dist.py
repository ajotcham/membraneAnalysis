...
def get_residence_time_and_y_diff(x,rt_dist):
    ...
    for i in range(len(boundsDF)-1): 
        if (cutDF[boundsDF[i]] < cutDF[boundsDF[i+1]]):
            ...
            times.append([start, stop, stop-start, y_avg, x_dist, y_dist, z_dist])
    if not times:
        times=None
    return times
...
for pressure_diff in pressure_diffs:
    ...
    del rtDF
    gc.collect()
    rearr_rtDF=pd.DataFrame(rearr_rtDF,columns=['mol','start','end','Residence time','avg absydiff','x dist','y dist','z dist',]).set_index('mol')
    ...
    rearr_rtDF.to_csv('./'+stage+'_csv_rt/64x_'+gap_size+'_rt_dist.csv')
