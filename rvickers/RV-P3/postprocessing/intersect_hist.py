...
for file in os.listdir(parentDir):
    ...
    allxys=np.array(molDF.T.droplevel(0).loc['thetax'].values).flatten()
    allxzs=np.array(molDF.T.droplevel(0).loc['thetay'].values).flatten()
    allyzs=np.array(molDF.T.droplevel(0).loc['thetaz'].values).flatten()
    del molDF
    gc.collect()
    ...
    thetaDF = pd.DataFrame([allxcorr,allycorr,allz,allxys,allxzs,allyzs]).T
    thetaDF.columns = ['xcorr','ycorr','z','thetax','thetay','thetaz']
    ...
    thetahist_tot.to_csv('./'+stage+'_rotation/64x_'+gap_size+'_rotation.csv')
    print('finished: '+file)
