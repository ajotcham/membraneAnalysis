...
for pressure_diff in pressure_diffs:
    ...
    vDFbinned = vDF.apply(pd.cut,bins=np.linspace(0,0.02,2000),labels=False)
    ppDF = pd.concat([vDFbinned,tubeDF], axis=1).sort_index(axis=1)
    maxwellDF_in = ppDF.groupby(level=0, axis=1).apply(get_maxwell_dist_ng)
    maxwellDF_out = ppDF.groupby(level=0, axis=1).apply(get_maxwell_dist_out)
    maxwellDF_in.to_csv('./maxwell/pagap'+gap_size+'_nve'+pressure_diff+'_in.csv')
    maxwellDF_out.to_csv('./maxwell/pagap'+gap_size+'_nve'+pressure_diff+'_out.csv')